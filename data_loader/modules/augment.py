...
class RandomNoise:
    ...
    def __call__(self, data: dict):
        ...
        if random.random() > self.random_rate:
            return data
        data['img'] = (random_noise(data['img'], mode='gaussian', clip=True) * 255).astype('img'.dtype)
        return data
...
#         # angle = np.random.choice([-90, 0, 90], 1, p=[0.25, 0.5, 0.25])   # 這裡改了只轉指定角度
#         # angle = np.random.choice(self.degrees[0], self.degrees[1], 1, p=[0.5, 0.5])   # 這裡改了只轉指定角度
